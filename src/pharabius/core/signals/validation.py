...
def validate_governed_signal(signal: GovernedSignal) -> SignalValidationResult:
    ...
    if not signal.signal_id:
        _add(
            INV_005_SIGNAL_ID_DETERMINISTIC.code,
    ...
    if signal.family.value not in _VALID_FAMILIES:
        _add("SIG-FAMILY", SignalValidationSeverity.CRITICAL, f"Invalid family: {signal.family}")

    # Kind
    if not signal.kind:
        _add("SIG-KIND", SignalValidationSeverity.CRITICAL, "Signal kind is empty")
    ...
    if not signal.category:
        _add("SIG-CATEGORY", SignalValidationSeverity.WARNING, "Signal category is empty")

    # Severity
    if not signal.severity:
        _add("SIG-SEVERITY", SignalValidationSeverity.WARNING, "Signal severity is empty")
    ...
    if not signal.confidence:
        _add("SIG-CONFIDENCE", SignalValidationSeverity.WARNING, "Signal confidence is empty")
    ...
    # Title
    if not signal.title:
        _add("SIG-TITLE", SignalValidationSeverity.CRITICAL, "Signal title is empty")

    # Summary
    if not signal.summary:
        _add("SIG-SUMMARY", SignalValidationSeverity.WARNING, "Signal summary is empty")

    # Explanation
    if not signal.explanation:
        _add("SIG-EXPLANATION", SignalValidationSeverity.INFO, "Signal explanation is empty")

    # FINDING must have evidence (INV_006)
    if signal.disposition == SignalDisposition.FINDING and not signal.evidence_ids:
        _add(
            INV_006_PROMOTED_FINDING_HAS_EVIDENCE.code,
            SignalValidationSeverity.CRITICAL,
    ...
    if signal.disposition == SignalDisposition.SUPPRESSED:
        _add(
            INV_004_SUPPRESSED_DIAGNOSTIC_ONLY.code,
            SignalValidationSeverity.INFO,
